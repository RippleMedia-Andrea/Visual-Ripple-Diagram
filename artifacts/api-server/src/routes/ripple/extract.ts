...
router.post("/journeys/:id/discoveries/:stage", requireAuth, async (req, res, next): Promise<void> => {
  try {
    ...
    const response = await anthropic.messages.create({ model:"claude-sonnet-4-6", max_tokens:2000, system:"You are organizing what a person shared during a purpose-discovery conversation. Only include things the person actually said or clearly confirmed. Use their own words whenever possible. Do not invent, embellish, or add interpretation that wasn't discussed. Keep each list item short (2–8 words). If something wasn't covered, leave it empty.", messages:[{role:"user",content:`Transcript:\n${messages.map((m)=>`${m.role}: ${m.content}`).join("\n")}`}], tools:[{name:"extract_discoveries",description:"Return structured discoveries.",input_schema:schemas[stage] as never}], tool_choice:{type:"tool",name:"extract_discoveries"} });
    ...
    if (stage === "reveal") { await db.delete(storyCards).where(eq(storyCards.journeyId, journey.id)); await db.insert(storyCards).values(data.stories.map((s:any,i:number)=>({...s,journeyId:journey.id,position:i}))); }
    if (stage === "identify") { await db.delete(purposeThemes).where(eq(purposeThemes.journeyId, journey.id)); await db.insert(purposeThemes).values(data.themes.map((t:any,i:number)=>({...t,journeyId:journey.id,position:i}))); }
    if (stage === "personalize") await db.update(journeys).set({season:data.season,updatedAt:new Date()}).where(eq(journeys.id,journey.id));
    if (stage === "live") await db.update(journeys).set({actionPlan:data.actionPlan,updatedAt:new Date()}).where(eq(journeys.id,journey.id));
    res.json(data);
  } catch (e) { next(e); }
});
